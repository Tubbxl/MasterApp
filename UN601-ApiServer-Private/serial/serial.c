...
int Lib_UartInit(unsigned char ComID,unsigned int Boudrate)
{
	...
	else if(ComID == IMX6UL_COM3)
		sprintf(com_path, "/dev/ttymxc3");
...
int Lib_UartInitEx(unsigned char ComID, unsigned char* comParam)
{
  	int iret = -1;
	unsigned char com_path[20];

	printf("Enter---Lib_UartInitEx\n");
	...
	memset(com_path, 0, 20);
	if(ComID == IMX6UL_COM0)
	sprintf(com_path, "/dev/ttymxc0");

    char szComParam[4][24] = {0};
	char *token= strtok(comParam,",");
    int i = 0;
	for(i = 0; i < 4; i++)
	{
	    ...
	    if(NULL != token)
	    {
        	strcpy(szComParam[i], token);
			
			token=strtok(NULL,",");
	    }
	}

	printf("%s port = %d com = %s \n", __FUNCTION__, 0, com_path);
	...
	int baudrate = atoi(szComParam[0]);
   	printf("Lib_UartInitEx---baudrate: [%d]\n", baudrate);
	
	iret = set_speed(com_fd[ComID], baudrate);
	printf("Lib_UartInitEx---set_speed iret: [%d]\n", iret);
	if(SUCCESS != iret)
	{
	   return iret;
	}

    int dataBits = (szComParam[1][0] - 0x30);

	printf("Lib_UartInitEx---dataBits: [%d]\n", dataBits);
	
	int parity = szComParam[2][0];

	printf("Lib_UartInitEx---parity: [%c]\n", szComParam[2][0]);

	printf("Lib_UartInitEx---parity: [%d]\n", parity);
	
	int stopBits = (szComParam[3][0] - 0x30);

	printf("Lib_UartInitEx---stopBits: [%d]\n", stopBits);
	
	return set_Parity(com_fd[ComID], dataBits, stopBits, parity);
}
...
int Lib_ScanUartRecv(unsigned char ComID,unsigned char *strBuf, unsigned short *p_read_len, unsigned short timeout,unsigned short nbyte)
{
    ...
    *p_read_len = 0;
  	while(nleft > 0) 
  	{
		...
		if (nRet > 0)
	    {
			...
			cntlen += nread;
  			strBuf += nread;
		if(((*(strBuf-2)==0x0d))&&((*(strBuf-1)==0x0a))) break;
		}
		else if (nRet == 0) 
		{
...
int LOGD_HEX(unsigned char *hexString,int HexLen)
{
	int i;
	char buf[30000]={0};
	for(i=0;i<HexLen;i++) sprintf(buf,"%s%02x ",buf,hexString[i]);
	printf("%s\n",buf);
}
...
int Crc16SendPacket(unsigned char *pbySendData, unsigned short wDataLen, unsigned short wCmd)
{
	...
	//for(i=0;i<wDataLen+7;i++) LOGD("%02x ",abyBuff[i]);
	LOGD_HEX(abyBuff,wDataLen+7);
#endif
	//Lib_UartReset(COMMAND_COM);
	RecvCrcPack_Flag = 0;
...
volatile long long Timer[10];
volatile long long Count[10];
...
{
    Timer[TimerNo] = sys_clock();
    Count[TimerNo] = count * 100;
}
...
unsigned long s_CheckTimer(unsigned char TimerNo)
{
    long long TimerTemp;
    TimerTemp = sys_clock();
    if (TimerTemp - Timer[TimerNo] >= Count[TimerNo])
        return 0;
    return 1;
}
...
int Crc16RecvPacket(unsigned char *pbyRecvData, unsigned short *pwPacketetLen,unsigned short *pwCmd, unsigned long dwTimeoutMs)
{
	...
	while (1)
	{
		...
		if(RecvCrcPack_Flag)
		{
			*pwCmd = g_abyRecvBuff_FromSecChip[1]*256 + g_abyRecvBuff_FromSecChip[2];
			*pwPacketetLen = g_abyRecvBuff_FromSecChip[3]*256 + g_abyRecvBuff_FromSecChip[4];// + 7;
			memcpy(pbyRecvData,g_abyRecvBuff_FromSecChip,*pwPacketetLen+7);
#ifdef COM_PACKET_DEBUG
	//for(i=0;i<wDataLen+7;i++) LOGD("%02x ",abyBuff[i]);
	LOGD_HEX(pbyRecvData,*pwPacketetLen+7);
#endif
			RecvCrcPack_Flag = 0;
			return 0;
		}
...
#ifdef COM_PACKET_DEBUG
	LOGD_HEX(pbyRecvData,wPacketLen+7);
#endif
#endif 
    return 0;
}

int RecvOneByte(unsigned char *pbyRecvData,unsigned long dwTimeoutMs)
{
    int i,iRet;
    unsigned short recv_len;
...
int RecvPacket(unsigned char *pbyRecvData, unsigned short *pwPacketetLen,unsigned short *pwCmd, unsigned long dwTimeoutMs)
{
	unsigned char buff[100];
    unsigned char abyCrc[2];
    int i,iRet;
...
{
	Lib_UartSend(COMMAND_COM,&ch,1);
}
int PortRecv(unsigned char * ch,WORD ms)
{
	unsigned short len;
	Lib_UartRecv(COMMAND_COM,ch,&len,ms,1);
	if(len==1) 
		return 0;
	return 1;
}
...
int RecvPack(BYTE *pbyRecvBuff, DWORD *pdwRecvLen)
{
    ...
    for (i=1; i<4; i++)
    {
        if (0 != PortRecv(pbyRecvBuff+i,100))
        {
            return 2;
        }
    }
    *pdwRecvLen = ((DWORD)pbyRecvBuff[2])*256 + (DWORD)pbyRecvBuff[3];
    ...
    {
        if(0 != PortRecv(pbyRecvBuff+i+4,100))
        {
            return 4;
        }
    }
