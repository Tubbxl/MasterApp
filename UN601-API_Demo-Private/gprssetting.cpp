...
void Gprssetting::on_connect_clicked()
{
   ...
   csq= DLL_PppCheckCsq();
    ...
    ret = DLL_PppGetIMEI(imei);
    if(ret == 0)
    {
        QString str;
        str.sprintf("The IMEI is: %s",imei);
        ui->textBrowser->append(str);
    }
    //ui->textBrowser->clear();
   ...
   if(ret == 0)
   {
      ...
      // ui->connect->setText("pass");
       ...
       ui->textBrowser->append("The Gateway is:"+QString(QLatin1String((char*)gate)));
        int ret = DLL_PppStatus();
        if(ret == 0)
            ui->textBrowser->append("PPP Online");
        update();
        QThread::sleep(1);
       ui->textBrowser->append("Start distconnect PPP");
       DLL_Pppdisconnect();
       QThread::sleep(1);
       ret = DLL_PppStatus();
       if(ret==0)
           ui->textBrowser->append("PPP Online. Distconnect PPP fail");
       else
           ui->textBrowser->append("PPP Offline. Distconnect PPP success");
       ui->textBrowser->append("Start Reset PPP");
       ret =DLL_PppReset();
       update();
       if(ret == 0)
