...
void Wifisetting::get_pswd(QString *temp)
{
    ...
    //clock ssid
   /* mPromptMsgBox = new QMessageBox(QMessageBox::Question,QString(tr("Lock SSID")),QString(tr(" Do you want to lock the  SSID?")),QMessageBox::Yes| QMessageBox::No,this);
    mPromptMsgBox->setStyleSheet("font: 14pt;background-color:rgb( 0,220, 0)");
    mPromptMsgBox->setModal(false);
    int result = mPromptMsgBox->exec();
    mPromptMsgBox->show();*/
     QMessageBox::StandardButton   result;
     result = QMessageBox::question(mPtrContainer, tr("Lock SSID"),QString(tr("Do you want to lock the  SSID?")),QMessageBox::Yes | QMessageBox::No,QMessageBox::Yes);
    if (result == QMessageBox::No)
        {
              DLL_WifiLockSSID(NULL);
              qDebug("NO");
        }
        else if (result == QMessageBox::Yes)
        {
               DLL_WifiLockSSID(&work[i]);
                qDebug("YES");
        }
    //end
...
void Wifisetting::get_network(T_WLANDEV *network, int *num)
{
    ...
    if(Cnt<0)
    {
       ui->textBrowser->append("Scanning  network fail.please retry");
       return;
    }
    while(Cnt--)
    {
...
void Wifisetting::isconnect(int ret)
{
    ...
    if(ret == 0)
    {
        ...
        // ui->OK->setText("ping");
         ...
         //ui->Setting->show();
        pingtestflag = 1;
        T_WLANSTATE paWlanStatus;
        DLL_WifiGetNetworkStatus(&paWlanStatus);
        ui->textBrowser->append("The wpa_state is:"+QString(QLatin1String((char*)paWlanStatus.wpa_state)));
        DLL_WifiDhcp();
        ...
        //ui->textBrowser->append("Start distconnect wifi");
        //DLL_WifiDisConnectNetwork();
      //  QThread::sleep(3);
        //memset(&paWlanStatus,0x00,sizeof(paWlanStatus));
        //DLL_WifiGetNetworkStatus(&paWlanStatus);
        //ui->textBrowser->append("The wpa_state is:"+QString(QLatin1String((char*)paWlanStatus.wpa_state)));
        ui->Setting->show();
